...
angular.module('spock.common.security.personaProvider', [])
...
.provider('persona', [
  function () {
      ...
      return {
        ...
        verify:function () {
          ...
          navigator.id.get(function (assertion) {
            $http.post(conf.baseUrl + '/persona/verify', {assertion:assertion})
        ...
        logout:function () {
          return $http.post(conf.baseUrl + '/persona/logout').then(function (response) {
            if (response.data.status != 'okay') {
        ...
        status:function () {
          return $http.post(conf.baseUrl + '/persona/status').then(function (response) {
    ...
    this.setBaseUrl = function (value) {
      return this.conf.baseUrl = value;
    };
...
.controller('PersonaCtrl', ['$scope', 'persona', 
  function PersonaCtrl($scope, persona) {
    // initialize properties
    angular.extend($scope, { verified:false, error:false, email:'' });
   
    $scope.verify = function () {
      persona.verify().then(function (email) {
    ...
    $scope.status = function () {
      persona.status().then(function (data) {
